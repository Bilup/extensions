...
class HomepageFile extends BuildFile {
  ...
  getRunExtensionURL(extensionSlug) {
    return `https://editor.bilup.org/editor?extension=${this.getFullExtensionURL(
      extensionSlug
    )}`;
  }
  ...
  getRunSampleURL(sampleFile) {
    ...
    return `https://editor.bilup.org/editor?project_url=${this.host}${path}`;
  }
...
class SitemapFile extends BuildFile {
  ...
  read() {
    ...
    xml += Object.keys(this.build.files)
      ...
      .map((path) => `https://extensions.bilup.org${path}`)
